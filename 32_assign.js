...
var current_users = ["admin", "eric", "jhon", "farooq", "salman"];
var new_users = ["hasan", "Admin", "eric", "jhon", "Farooq", "sameer"];
var current_user = current_users.map(function (user) { return user.toLowerCase(); });
var new_user = new_users.map(function (user) { return user.toLowerCase(); });
for (var i = 0; i < new_user.length; i++) {
    if (current_user.includes(new_user[i]))
        console.log("Username \"".concat(new_users[i], "\" is already taken. Please enter a new name."));
    else {
